...
import boss.portal.filter.CustomPostProcessor;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        LogoutConfigurer<HttpSecurity> httpSecurityLogoutConfigurer = http.cors().and().csrf().disable()
                ...
                //.antMatchers("/users/userList").hasRole("USER")
...
//                .and()
                ...
                .formLogin()
                .loginPage("/login")
                .defaultSuccessUrl("/index")//默认登录成功跳转页面
                .failureUrl("/login?error").permitAll()//登录失败跳转页面
                .and()
...
//                .logoutSuccessHandler(customLogoutSuccessHandler)
    ...
    public void configure(AuthenticationManagerBuilder auth) throws Exception {
        // 使用自定义身份验证组件
        auth.authenticationProvider(new CustomAuthenticationProvider(userDetailsService, bCryptPasswordEncoder,userMapper,roleMapper,userRoleMapper));
    }
}
